<aside class="menu-sidebar3 js-spe-sidebar" style="position:fixed;">
    <div class="table-responsive table-data">
        <nav class="navbar-sidebar2 navbar-sidebar3">
            <ul class="list-unstyled navbar__list">
                <li class="active has-sub">
                    <a href="{{url('buat_resep/minuman')}}">Buat Resep</a>
                </li>
            </ul>
        </nav>
        <br>
        ...
        <nav class="navbar-sidebar2 navbar-sidebar3">
            <ul class="list-unstyled navbar__list">
                <li name="search-option" id="top" value="top" class="{{Request::path()=='minuman' ? 'sub-cat' : ''}}">
                    <a href="{{url('minuman')}}">Top</a>
                </li>
                <form action="{{route('minuman.filter')}}" method="get" id="form2">
                ...
                <form action="{{route('minuman.filter')}}" method="get" id="form3">
        ...
        <nav class="navbar-sidebar2 navbar-sidebar3">    
            <ul class="list-unstyled navbar__list">
                <?php $category = \App\SubCategory::where('id_Category','1')->get()?>
                @foreach($category as $row)
                    <?php $i = 0?>
                    <form action="{{route('minuman.filter')}}" method="get" id='form-{{$row->id_sub_post}}'>
