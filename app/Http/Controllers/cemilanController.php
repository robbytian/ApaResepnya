...
class cemilanController extends Controller
{
    public function index(){
        $data['cemilan'] = \App\Post::where('id_category','3')->where('status','=','1')->paginate(18);
        return view('cemilan/index')->with($data);
    }
    
    // public function buatresep(){
    //     return view('cemilan/buat_resep');
    // }
    ...
    //                 return redirect('cemilan/buat_resep')->with('error','Cover Tidak Boleh Kosong(Pilih 1, lewat URL atau memakai foto cover sendiri)');
    //             }
    //             if(!empty($request['cover']) && !empty($request['cover-link'])){
    //                 return redirect('cemilan/buat_resep')->with('error','Hanya Bisa memasukan 1 cover!(Pilih 1, lewat URL atau memakai foto cover sendiri)');
    //             }
    //             if(!empty($request['cover']) && empty($request['cover-link'])){
    //                 $photo = $request['cover'];
    ...
    //                     return redirect('cemilan/buat_resep')->with('error','URL yang anda masukan pada Cover bukanlah sebuah gambar!');
    //                 }
    //             }

    //             if(!empty($request['video']) && !empty($request['video-link'])){
    //                 return redirect('cemilan/buat_resep')->with('error','Hanya Bisa memasukan 1 video!(Pilih 1, lewat URL atau memakai video sendiri)');
    //             }
    //             if(!empty($request['video']) && empty($request['video-link'])){
    //                 $video = $request['video'];
    ...
    //                     return redirect('data_cemilan')->with('success','Resep Berhasil Dibuat');
    //                 }
    //                 else{
    //                     return redirect('cemilan/buat_resep')->with('error','Data Gagal Dibuat');
    //                 }
    //             }
            
    //         else{
    //             return redirect('cemilan/buat_resep')->with('error','Judul jangan sama dengan Post yang sudah anda upload!');
    //         }
    //     }
    //     else{
    //         $input = [];
    ...
    public function destroy(Request $request){
        $deleteCemilan=$request->input('konfirmasiDelete');
        $result = \App\Post::where('id_tutorial', $deleteCemilan)->first();
        $status = $result->delete();

        if($status) return redirect('data_cemilan')->with('success','Resep berhasil dihapus');
        else return redirect('data_cemilan')->with('error','Resep gagal dihapus');
      }
    ...
    public function filter(Request $request){
        ...
        $data['filter']= $filter;
        if($filter == 'top'){
        $data['cemilan_filter'] = \App\Post::where('id_category','3')->where('status','=','1')->orderBy('created_at','DESC')->paginate(18);
        }
        if($filter == 'terbaru'){
        $data['cemilan_filter'] = \App\Post::where('id_category','3')->where('status','=','1')->orderBy('created_at','DESC')->paginate(18);
        }
        ...
        $category = \App\SubCategory::where('id_category',3)->get();
        foreach($category as $row){
            if($filter == $row->nama){
                $data['cemilan_filter'] = \App\Post::where('id_sub_post',$row->id_sub_post)->where('status','=','1')->paginate(18);
            }
        }
        return view('cemilan/result')->with($data);
    }
